...
func filter(labels map[string]string, hostname string) (string, string, string, error) {
	...
	// Don't log the name instance this is running on (meant to be run in a container).
	if labels[labelPod] == hostname {
		return "", "", "", fmt.Errorf("not logging self")
	}
